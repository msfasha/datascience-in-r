...
```{r} seq_1_to_10 <- 1:10 print(seq_1_to_10)}
```
...
```{r} seq_by_2 <- seq(from = 1, to = 10, by = 2) print(seq_by_2)}
```

-   Create a sequence from 5 to 10, consisting of 3 equally spaced numbers:

```{r} seq_length_out <- seq(from = 5, to = 10, length.out = 3) print(seq_length_out)}
```
...
```{r} rep_times <- rep(x = 1:3, times = 3) print(rep_times)}
```

-   Repeat each number in the sequence 1, 2, 3, three times:

```{r} rep_each <- rep(x = 1:3, each = 3) print(rep_each)}
```
...
```{r} combined_seq <- c(seq(1, 5), seq(8, 10)) print(combined_seq)}
```
